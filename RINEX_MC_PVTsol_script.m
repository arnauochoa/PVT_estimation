...
for con = 1:Nconst
    ...
    for  epoch = 1:Nepoch
        ...
        [PVT(epoch, :, con), A, Tcorr(epoch, con), Pcorr(epoch, con)]  =   PVT_recLS(pr, sats, TOW(epoch), eph{con}, iono{con}, Nit, PVT0);
...
filename = sprintf('Capt/mult_1/err_XYZ_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
%
% -- RMS evolution 
...
filename = sprintf('Capt/mult_1/rms_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
%
% -- Error in time
...
title(sprintf('Evolution of the error in time (GPS weight = %1.2f, GAL weight = %1.2f)\n', weight(1), weight(2)));
filename = sprintf('Capt/mult_1/terr_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
%
% -- # satellites in view
...
filename = sprintf('Capt/mult_1/nsat_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
%
...
xlabel('Time of the Week (s)');
title(sprintf('RMS evolution (GPS weight = %1.2f, GAL weight = %1.2f)\n', weight(1), weight(2)));
filename = sprintf('Capt/mult_1/rms-nsat_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
%
...
filename = sprintf('Capt/mult_1/gdop-nsat_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
%
...
filename = sprintf('Capt/mult_1/pdop-nsat_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
%
...
filename = sprintf('Capt/mult_1/tcorr-pcorr_%u_%u.jpg', 100*weight(1), 100*weight(2));
saveas(fig, filename);
